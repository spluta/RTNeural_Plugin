...
class PreEmph(nn.Module):
    ...
    def forward(self, output, target):
        ...
        output = torch.cat((torch.zeros(self.zPad, output.shape[1], 1, device=output.device), output))
        target = torch.cat((torch.zeros(self.zPad, target.shape[1], 1, device=target.device), target))
...
class LossWrapper(nn.Module):
    def __init__(self, losses, pre_filt, device):
        super(LossWrapper, self).__init__()
        loss_dict = {'ESR': ESRLoss().to(device), 'DC': DCLoss().to(device)}
        if pre_filt:
            pre_filt = PreEmph(pre_filt).to(device)
            loss_dict['ESRPre'] = lambda output, target: loss_dict['ESR'].forward(*pre_filt(output, target))
            
        else:
            loss_dict['ESRPre'] = lambda output, target: loss_dict['ESR'].forward(output, target)
